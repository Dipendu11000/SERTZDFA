...
class HealthcareAgentSystem:
    ...
    def _create_report_generator_tool(self):
        ...
        def generate_combined_report_llm(treatments: List[Dict[str, Any]], risks: List[Dict[str, Any]], revenues: List[Dict[str, Any]]) -> str:
            ...
            - Clear headings for each treatment
            - Multi-paragraph, business-style summaries
            - Risk and revenue sections as Markdown tables
            - Bullet points, bold, subheadings, etc.
            - Output ONLY Markdown, no JSON
            """
            try:
                prompt = (
                    "You are a business analyst and medical writer. Given the following treatments, risks, and revenues, generate a detailed, business-style, Markdown-formatted report. For each treatment, do the following: "
                    "1. Group and merge all semantically similar treatments from all sources.\n"
                    "2. For each treatment, write at least a page of detailed, business-style description (multi-paragraph, not bullets).\n"
                    "3. For each treatment, provide a risk assessment as a Markdown table (Metric | Value | Explanation/Logic).\n"
                    "4. For each treatment, provide a customer/revenue impact section as a Markdown table (Segment | Number | Logic/Explanation).\n"
                    "5. Use clear headings for each treatment, subheadings for sections, bullet points, bold, and other formatting for clarity.\n"
                    "6. Output ONLY Markdown, no JSON, no code blocks.\n"
