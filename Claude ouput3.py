    ...
    async def process_treatment(self, treatment_id: str, input_files: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            for file_path in input_files:
                file_data = file_reader_tool(file_path=file_path)
                if "error" not in file_data:
                    treatment_data.raw_content += file_data["content"] + "\n"
            
            # Document structuring phase
            document_structuring_tool = self._create_document_structuring_tool()
            structured_data = document_structuring_tool(
            ...
            risk_analysis_tool = self._create_risk_analysis_tool()
            ...
            revenue_analysis_tool = self._create_revenue_analysis_tool()
            revenue_analysis = revenue_analysis_tool(
                treatment_data=structured_data,
            ...
            # Customer segmentation
            customer_segmentation_tool = self._create_customer_segmentation_tool()
            segmentation = customer_segmentation_tool(
                treatment_analysis={
                    "treatment_id": treatment_id,
            ...
            document_aggregation_tool = self._create_document_aggregation_tool()
            aggregated_doc = document_aggregation_tool(
                analysis_results={
                    "treatment_id": treatment_id,
                    "risk_analysis": risk_analysis,
                    "revenue_analysis": revenue_analysis,
            ...
            export_result = word_export_tool(
...
if __name__ == "__main__":
    if "ipykernel" in sys.modules or "IPython" in sys.modules:
        ...
        import asyncio
        try:
            # Try to use top-level await (works in Jupyter cells)
            get_ipython()
            await main()
        except Exception:
            # If not at top-level, fallback to ensure_future
