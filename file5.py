...
class HealthcareAgentSystem:
    ...
    def _create_word_export_tool(self):
        import re
        @tool
        def export_to_word(markdown_report: str, output_path: str) -> Dict[str, Any]:
            """
            Export a Markdown-formatted, business-style report to a visually appealing Word document.
            """
            try:
                doc = docx.Document()
                ...
                for line in lines:
                    ...
                    if re.match(r'^#{1,6} ', line):
                        ...
                        text = line[level+1:].strip() if line.startswith('# ') else line[level:].strip()
                        doc.add_heading(text, level=level)
                        table = None
                    # Table header
                    elif '|' in line and re.match(r'^[^|]*\|[^|]*\|', line) and ('---' not in line):
                    ...
                    # Bold
                        ...
                        run.bold = True
                        table = None
                    # Graphical trends (unicode bars/sparklines or chart placeholders)
    ...
    def _create_report_generator_tool(self):
        @tool
        def generate_combined_report_llm(treatments: List[Dict[str, Any]], risks: List[Dict[str, Any]], revenues: List[Dict[str, Any]]) -> str:
            """
            Use LLM to generate a detailed, business-style, Markdown-formatted report with:
            - Clear headings for each treatment (merge/compare all sources, no 'treatment 1', use real names)
            - Multi-paragraph, business-style summaries (at least a page per treatment, no 'our', no quarterly, no placeholders)
            - Risk and revenue sections as Markdown tables, with all metrics and explanations LLM/logically generated (not hardcoded)
            - Customer/revenue impact as Markdown tables, with segmentation, numbers, logic, and business context
            - Bullet points, bold, subheadings, and simple graphical trends (unicode bars/sparklines or chart placeholders)
            - Use only information from the files, merge/compare, and reason out all metrics and explanations
            - Use business/competitive intelligence language, not internal/ownership
            - Output ONLY Markdown, no JSON, no code blocks, no 'note', no 'placeholder', no 'quarterly', no 'our'
            """
            try:
                prompt = (
                    "You are a business analyst and competitive intelligence writer. Given the following scraped data from various competitor and industry sources (HTML, PDF, DOCX), generate a detailed, business-style, Markdown-formatted report for CareCredit business users. For each treatment or service found (merging/combining all semantically similar mentions from all sources):\n"
                    "1. Use the real treatment/service name as the heading.\n"
                    "2. Write at least a page of detailed, business-style, multi-paragraph summary for each treatment/service. Do NOT use 'our', 'we', 'quarterly', 'note', or 'placeholder' language.\n"
                    "3. For each treatment, provide a risk assessment as a Markdown table (Metric | Value | Explanation/Logic), with all values and explanations reasoned out by the LLM or calculated logically from the data.\n"
                    "4. For each treatment, provide a customer/revenue impact section as a Markdown table (Segment | Number | Logic/Explanation), with segmentation, numbers, and logic reasoned out by the LLM or calculated from the data.\n"
                    "5. Use bullet points, bold, subheadings, and simple graphical trends (unicode bars/sparklines or chart placeholders) for visual appeal.\n"
                    "6. Use only information from the files, merge/compare, and reason out all metrics and explanations.\n"
                    "7. Use business/competitive intelligence language, not internal/ownership.\n"
                    "8. Output ONLY Markdown, no JSON, no code blocks, no 'note', no 'placeholder', no 'quarterly', no 'our'.\n\n"
                    f"Scraped Data (HTML/PDF/DOCX):\n{json.dumps(treatments, indent=2)}\n\n"
    ...
    async def process_treatment(self, treatment_id: str, input_files: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            export_result = word_export_tool(markdown_report=final_report, output_path=output_path)
