...
class HealthcareAgentSystem:
    ...
    def _create_word_export_tool(self):
        ...
        def export_to_word(aggregated_doc: Dict[str, Any], output_path: str) -> Dict[str, Any]:
            """
            Export aggregated document or report to Word format.
            """
            try:
                doc = docx.Document()
                # If 'report' key exists, treat as plain text/markdown
                if "report" in aggregated_doc:
                    for line in aggregated_doc["report"].splitlines():
                    ...
                    doc.add_paragraph(json.dumps(aggregated_doc, indent=2))
    ...
    def _create_report_generator_tool(self):
        ...
        def generate_combined_report_llm(treatments: List[Dict[str, Any]], risks: List[Dict[str, Any]], revenues: List[Dict[str, Any]]) -> str:
            """
            Use LLM to generate a final, human-friendly, well-formatted report with all sections, explanations, and summaries.
            """
            try:
                prompt = (
                    "You are a business analyst. Compose a comprehensive, human-friendly, markdown-formatted report for the following treatments, "
                    "including detailed descriptions, risk assessments (with parameter explanations and logic), and revenue analyses (with business logic). "
                    "Group semantically similar treatments, and make the report readable for business users.\n\n"
                    f"Treatments:\n{json.dumps(treatments, indent=2)}\n\n"
    ...
    async def process_treatment(self, treatment_id: str, input_files: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            export_result = word_export_tool(aggregated_doc={"report": final_report}, output_path=output_path)
